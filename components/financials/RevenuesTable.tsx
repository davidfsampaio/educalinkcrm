...
interface RevenuesTableProps {
    ...
    onDelete: (revenueId: number) => void;
}

const RevenuesTable: React.FC<RevenuesTableProps> = ({ revenues, onEdit, onDelete }) => {
    return (
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <ProtectedComponent requiredPermission='manage_revenues'>
                                <div className="flex items-center space-x-3">
                                    <button onClick={() => onEdit(revenue)} className="text-brand-primary hover:text-sky-700" title="Editar">
                                        <EditIcon className="w-5 h-5" />
                                    </button>
                                    <button onClick={() => onDelete(revenue.id)} className="text-red-500 hover:text-red-700" title="Excluir">
